...
bool golden_modulation(int bits[NUM_BITS], float complex symbols[NUM_SYMBOLS])
{
    int num;
    int ibits = 0;
...
float calculate_mean_power (const float complex symbols[NUM_SYMBOLS])
{
...
bool awgn_channel(float complex in_symbols[NUM_SYMBOLS], float complex out_symbols[NUM_SYMBOLS])
{
...
int main(void)
{
    bool error = false; //Comprobación de error
    ...
    //TX SÍMBOLOS POR UN CANAL AWGN SIMULADO
    float complex rx_symbols[NUM_SYMBOLS];
    error = awgn_channel(tx_symbols, rx_symbols);
