...
bool generate_preamble_bpsk(float complex preamble[PREAMBLE_LEN]) {
    // Generar preámbulo BPSK simple basado en la palabra de sincronización
    uint32_t sync = PREAMBLE_SYNC_WORD;

    for (int i = 0; i < PREAMBLE_LEN; i++) {
        // Usar diferentes bits de la palabra de sync para cada símbolo
        uint8_t bit = (sync >> (i % 32)) & 1;

        // Modulación BPSK: bit 0 -> +1, bit 1 -> -1
...
bool add_preamble_to_frame(const float complex ofdm_symbols_with_cp[N_FFT + CP_LEN],
    ...
    // Añadir preámbulo al inicio de la trama
    for (int i = 0; i < PREAMBLE_LEN; i++) {
        frame_with_preamble[i] = preamble[i];
    }

    // Añadir símbolos OFDM después del preámbulo
...
float calculate_correlation(const float complex received_signal[PREAMBLE_LEN + N_FFT + CP_LEN],
    ...
    for (int i = 0; i < PREAMBLE_LEN; i++) {
        if (start_index + i < PREAMBLE_LEN + N_FFT + CP_LEN) {
            // Correlación simple (solo parte real para BPSK)
            correlation_real += crealf(received_signal[start_index + i]) * crealf(local_preamble[i]);
        }
    }

    return correlation_real / PREAMBLE_LEN; // Normalizar
}
...
bool detect_frame_start_improved(const float complex received_signal[PREAMBLE_LEN + N_FFT + CP_LEN],
    ...
    int best_index = -1;
    int search_window = N_FFT + CP_LEN; // Buscar en toda la ventana

    printf("Buscando preambulo...\n");

    // Buscar el preámbulo en la señal recibida
...
bool extract_ofdm_from_frame(const float complex frame_with_preamble[PREAMBLE_LEN + N_FFT + CP_LEN],
                             int frame_start_index, float complex ofdm_symbols_with_cp[N_FFT + CP_LEN]) {
    ...
    // Extraer símbolos OFDM (después del preámbulo)
    for (int i = 0; i < N_FFT + CP_LEN; i++) {
...
int main(void)
{
    ...
    while (run < total_transmissions)
    {
        ...
        if (error) {
            printf("Error añadiendo preambulo!\n");
            return -1;
        }
        ...
        if (error) {
            printf("Usando posicion por defecto después del preambulo.\n");
            frame_start_index = PREAMBLE_LEN; // Fallback
            used_fallback = true;
        } else {
        ...
        // 8. REMOVEMOS CP + APLICAMOS DFT + DEMAPEAMOS OFDM
        ...
        // Estrategia de validación mejorada
        ...
        if (run < total_transmissions - 1) {
            Sleep(500); // Reducido para pruebas
        }
        run++;
    }
